...
# with this one
# and then run it to install latest platform tools and so have a recent rustc version

# to run unit tests, simply use 
# cargo test
# because cargo test-sbf will insist on using old platfom tools
# but your local rustc version should be compatible with solana's 1.15.2, which is
# rustup install 1.66.1; rustup install 1.66.1
...
if [[ ! -e sbf-tools-$version.md || ! -e sbf-tools ]]; then
  (
    ...
    job="download \
           ...
           $version \
           platform-tools-$machinesbf.tar.bz2 \
           sbf-tools"
    get $version sbf-tools "$job"
  )
  exitcode=$?
